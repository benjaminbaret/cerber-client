...
int main() {
    ...
    do {
        ...
        curl_easy_setopt(curl, CURLOPT_URL, "http://localhost:3333/device/update/next");
        ...
        if (http_code == 200) {
            ...
            const gchar *uri = json_object_get_string_member(root, "uri");
            const gchar *size = json_object_get_string_member(root, "size");
            printf("Name: %s\n", name);
            printf("Uri: %s\n", uri);
            printf("Size: %s\n", size);
