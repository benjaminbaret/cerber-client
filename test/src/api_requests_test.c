...
START_TEST(test_api_patch_deploy_status)
{
    gchar *jwtToken = api_post_device_signin();
    ck_assert_msg(jwtToken != NULL, " Failed.");
    gboolean isDeployed = FALSE;

    glong http_code = api_patch_deploy_status(jwtToken, isDeployed);
    ck_assert_msg(http_code == 200, " Failed.");
}
END_TEST
...
Suite *api_requests_suite(void)
{
    ...
    tc = tcase_create("");
    tcase_add_test(tc, test_api_patch_deploy_status);
    suite_add_tcase(s, tc);
